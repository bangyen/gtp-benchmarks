(module zombie racket/base
  (#%module-begin
   (provide w0 world-on-mouse world-on-tick)
   (require (for-syntax
             racket/sequence
             racket/base
             syntax/parse
             racket/syntax))
   (require (only-in "image.rkt" empty-scene place-image circle))
   (require (only-in "math.rkt" min max abs sqr msqrt))
   (define WIDTH 400)
   (define HEIGHT 400)
     ...
     (lambda (msg)
       (cond
        ...
        ((equal? msg 'move-toward)
         (cons
          'move-toward
          (lambda (p) (new-horde ((zombies-move-toward undead) p) dead))))
   ...
   (define (new-mt-zombies)
     (lambda (msg)
       (cond
        ...
        ((equal? msg 'kill-all)
         (cons 'kill-all (lambda (dead) (new-horde (new-mt-zombies) dead))))
        (else (error 'zombies "unknown message")))))
   (define (new-zombie p)
     (lambda (msg)
       (cond
        ...
        ((equal? msg 'touching?)
         (cons 'touching? (lambda (q) (<= ((posn-dist p) q) ZOMBIE-RADIUS))))
        ((equal? msg 'move-toward)
         (cons
          'move-toward
          (lambda (q)
                ...
                (cond
                 ...
                 (else
                  ((posn-move this)
                   ...
                   0)))))))
          ((equal? msg 'move)
           (cons 'move (lambda (x2 y2) (new-posn (+ x x2) (+ y y2)))))
          ((equal? msg 'draw-on/image)
           (cons 'draw-on/image (lambda (img scn) (place-image img x y scn))))
          ((equal? msg 'dist)
           (cons
            'dist
            (lambda (p)
